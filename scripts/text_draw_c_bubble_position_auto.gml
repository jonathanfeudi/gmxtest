...
if !instance_exists(inst) {
    ...
    if (point_xpos > Tx && point_xpos+32 < Tx+Twidth) {
        ...
        draw_set_color(textbox[? "bgcolor"]); //"bgcolor"
        //draw_triangle(point_xpos,point_ypos,point_xpos+32,point_ypos,point_xpos+16,point_ypos+(point_spr*12),false); //point_spr*24
        draw_sprite(spr_talkerA,-1,point_xpos+16,point_ypos+(point_spr*12));
        draw_set_alpha(1);
    }
}
//Apply
textbox[? "__draw_x"] = Tx;
textbox[? "__draw_y"] = Ty+global.textbubble; // -0 // -96
textbox[? "__draw_width"] = Twidth;
textbox[? "__draw_height"] = Theight;
textbox[? "__draw_bubble_point_spr"] = point_spr; // -0
